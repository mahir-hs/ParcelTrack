...
| Courier observations → authoritative shipment | ✅ Done — separate `carrier.status.observed` topic, validated through the state machine, rejections logged not thrown |
| Tenant context outside an HTTP request | ✅ Done — `ITenantContextSetter`, scoped, set per Kafka message |
...
|---|---|---|
| ShipmentService.UnitTests | 86 | ✅ Domain + all 5 handlers |
...
**Verified 2026-09-04: solution builds clean (0 errors) on SDK 10.0.400; all 247 unit tests pass.** Integration tests not run (need Docker for Testcontainers).
